...
ssh -p $Port -i $SSHKey "$User@$HostName" "mkdir -p /opt/avtosrm"

Write-Host "=== Uploading files ==="
scp -P $Port -i $SSHKey avtosrm "$User@$HostName:/opt/avtosrm/"
scp -P $Port -i $SSHKey .env "$User@$HostName:/opt/avtosrm/"
scp -P $Port -i $SSHKey deploy/avtosrm.service "$User@$HostName:/tmp/avtosrm.service"

Write-Host "=== Installing systemd service ==="
ssh -p $Port -i $SSHKey "$User@$HostName" @"
